...
public class BotStarter implements Bot {
	...
	Reinforcement rBot = new Reinforcement(true,"states_10x10.txt");
	/**
...
//		System.err.printf("my hand is %s, opponent action is %s, pot: %d\n", handCategory, state.getOpponentAction(), state.getPot());
		...
		Card[] allMyCards = ObjectArrays.concat(hand.getCards(), state.getTable(), Card.class);
		double handEval = es.normalizedHandScore(allMyCards);
		double riskEval = rEval.riskEval(hand.getCards(), state.getTable());
		boolean action = rBot.playAction(handEval, riskEval);
		rBot.recordNetwork("states_10x10.txt");
		if(!action && state.getAmountToCall() == 0){
			rBot.setContradictAction(true);
			...
			double ourAmount = rBot.amountToPutIn(handEval, riskEval);
			double theirAmount = (double)state.getAmountToCall() / state.getPot();
			if(Math.abs(ourAmount - theirAmount) < 0.1){
				return new PokerMove(state.getMyName(), "call", state.getAmountToCall());
			}
			else{
				return new PokerMove(state.getMyName(), "raise", (int)((ourAmount/theirAmount)*state.getAmountToCall()));
			}
		}
